...
def _sitk():
    try:
        ...
        raise ImportError("intensity registration needs SimpleITK: uv sync --extra itk (or --extra seg)") from e
    return sitk
...
def _to_sitk(vol: Volume, channel: int = 0, dtype=np.float32):
    ...
    if np.abs(off).max() > 1e-6 * max(1.0, np.abs(A).max()):
        raise ValueError("intensity registration needs an axis-aligned volume (apply the pre-align first)")
...
def fixed_mask_from_footprint(fixed: Volume, moving: Volume, lm_to_em: AffineTransform, margin_nm: float = 500.0):
...
def ncc_on_synthetic(fixed: Volume, moving: Volume, channel: int, lm_to_em: AffineTransform) -> float:
    ...
    lm = np.asarray(data[0].compute(), dtype=np.float32)
    c = np.asarray(cov.compute()) > 0
    if c.sum() < 100:
        return float("nan")
...
def z_scan(
    ...
    for zs in z_scales:
        for off in offsets_nm:
            M = M0.copy()
            M[0, :3] *= zs
            M[0, 3] = M0[0, 3] + float(off)
            out.append({"offset_nm": float(off), "z_scale": float(zs), "ncc": ncc_on_synthetic(fixed, moving, channel, AffineTransform(M))})
    return out
...
def register_affine(
    ...
    res = IntensityResult(
        ...
        info={"metric": metric, "mask_fraction": frac, "shrink": list(shrink), "sigmas": list(sigmas)},
    )
...
def register_bspline(
    ...
    reg.SetMetricFixedMask(mask)
    reg.SetInterpolator(sitk.sitkLinear)
    reg.SetOptimizerAsLBFGSB(gradientConvergenceTolerance=1e-5, numberOfIterations=int(iterations), maximumNumberOfCorrections=5, maximumNumberOfFunctionEvaluations=int(iterations) * 5, costFunctionConvergenceFactor=1e7)
    ...
    mapped = np.array([final.TransformPoint((float(p[2]), float(p[1]), float(p[0])))[::-1] for p in pts])
    values = (mapped - pts).reshape(*n, 3).astype(np.float32)
    dg = DisplacementGrid(lo, sp, values, lm_to_em.inverse())
    info = {"metric_before": before, "metric_after": after, "mesh": mesh, "stop": str(reg.GetOptimizerStopConditionDescription()), "mask_fraction": frac}
